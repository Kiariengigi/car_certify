...
const StepThree = ({ size = '50%', className = '', onDataChange, data, setData }) => {
  ...
  const [loggedDamages, setLoggedDamages] = useState([]);
  ...
  const handleSubmit = e => {
    ...
    setActivePart(null); // Deselect after adding
  };

  // FIX 1: Updated keys to match the specific strings used in the buttons ('Minor damage' vs 'Minor_Damage')
  const severityColors = {
    'Minor damage': '#08CB00',      // Green
    'Functional damage': '#CFFF04', // Neon Yellow/Green
    'Disabling damage': '#FFA239',  // Orange
    'Write off': '#F93827'          // Red
  };
  ...
  const PartGroup = ({ id, children }) => {
    ...
    // If severity exists, use that color. 
    // If it's just active (selected but no severity yet), use Blue. 
    // Otherwise black.
    const strokeColor = severity ? severityColors[severity] : (isActive ? '#007BFF' : 'black');
    
    // FIX 3: Visibility & Glow
    // If severity exists, use severity glow.
    // If active, use Blue glow.
    let glow = 'none';
    if (severity) {
       glow = `0 0 10px ${severityColors[severity]}`;
    } else if (isActive) {
       glow = '0 0 15px #007BFF'; // Bright Blue glow for selection
    }

    const styles = {
      cursor: 'pointer',
      stroke: strokeColor,
      strokeWidth: isActive || severity ? 5 : 2, // Thicker line if active or has damage
      opacity: (activePart && !isActive) ? 0.3 : 1, // Dim others more to make selection pop
  ...
  return (
    <div className={`relative ${className}`} onClick={() => setActivePart(null)}>
      {tooltip.visible && (
        <div
          style={{
            ...
            borderRadius: '8px',
            zIndex: 1000,
            boxShadow: '0 4px 15px rgba(0,0,0,0.2)' // FIX 4: Stronger shadow for floating menu
          }}
        >
          <div className="mb-2 font-bold text-gray-800">Select Severity</div>
          {['Minor damage', 'Functional damage', 'Disabling damage', 'Write off'].map(level => (
            <div key={level} style={{ display: 'block', marginBottom: '5px' }}>
              <button
                onClick={(e) => {
                    e.stopPropagation(); // Prevent closing when clicking button
                    handleSeveritySelect(level);
                }}
                className="px-3 py-2 rounded w-full text-left font-semibold border transition-transform hover:scale-105"
                style={{ 
                    backgroundColor: severityColors[level], 
                    // FIX 5: Dynamic Text Contrast. If Yellow/Green, use Black text. Else White.
                    color: level === 'Functional damage' ? 'black' : 'white',
      ...
      <div className="absolute top-0 left-0 p-4 pointer-events-none">
        <h3 className="text-lg font-bold text-gray-800 bg-white/90 backdrop-blur px-3 py-1 rounded border shadow-sm">
          {activePart ? activePart.toUpperCase() : 'TAP A VEHICLE PART'}
        </h3>
      </div>

      {/* Form for date input */}
      <div className="absolute bottom-4 left-4 bg-white p-4 rounded-lg shadow-lg border border-gray-200" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex gap-2">
            ...
            <button 
                ...
                // FIX 6: Disabled state styling if fields are missing
                disabled={!newDamage.location || !newDamage.severity || !newDamage.date}
                className={`px-4 py-1 rounded font-bold text-white transition-colors ${
                    (!newDamage.location || !newDamage.severity || !newDamage.date) 
        ...
        {newDamage.location && (
            <div className="text-xs text-blue-600 mt-1">
                Selected: {newDamage.location} ({newDamage.severity || 'No severity'})
            </div>
        )}
      </div>

      {/* Summary panel */}
      <div className="absolute bottom-4 right-4 bg-white p-4 rounded-lg shadow-lg border border-gray-200 max-h-64 overflow-y-auto w-80" onClick={e => e.stopPropagation()}>
        <h4 className="font-bold mb-3 border-b pb-1">Damage Summary</h4>
        {loggedDamages.length === 0 ? (
          <p className="text-gray-400 italic text-sm">No damages logged yet.</p>
        ) : (
          <ul className="space-y-2">
            {loggedDamages.map(d => (
              <li key={d.id} className="flex justify-between items-center bg-gray-50 p-2 rounded">
                <div>
                    <div className="font-bold text-sm">{d.location.toUpperCase()}</div>
                ...
                <div className="flex items-center gap-2">
                    <span 
                        ...
                        style={{ 
                            ...
                            color: d.severity === 'Functional damage' ? 'black' : 'white'
                        }}
                    >
                        {d.severity.replace(' damage', '').toUpperCase()}
                    </span>
                    <button
                        ...
                        title="Remove entry"
                    >
                        {/* SVG Trash Icon for better visuals than text button */}
      ...
      <svg
        ...
        className="vehicle-map" // Ensure your CSS doesn't override strokes
        onClick={(e) => { e.stopPropagation(); setActivePart(null); }}
      >
         {/* ... (Keep your PartGroups exactly as they were, they are fine) ... */}
         {/* FRONT */}
        <PartGroup id="Front" activePart={activePart}>
            {/* ... Paste your SVG paths for Front here ... */}
        ...
        {/* REAR */}
        <PartGroup id="Rear" activePart={activePart}>
             {/* ... Paste your SVG paths for Rear here ... */}
        ...
        {/* LEFT */}
        <PartGroup id="Left" activePart={activePart}>
            {/* ... Paste your SVG paths for Left here ... */}
        ...
        {/* RIGHT */}
        <PartGroup id="Right" activePart={activePart}>
            {/* ... Paste your SVG paths for Right here ... */}
